<div class="grid grid-cols-1 p-5">
  <div class="container">
    <div class="relative mx-auto max-w-4xl rounded-xl shadow-md p-3 bg-thirdclr">
      <button id="dropdownForum" data-dropdown-toggle="dropdownfrm-{{ $discuss->id }}" class="absolute text-white top-0 right-0 hover:bg-mainclr rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center" type="button">
        ...
        <!-- Dropdown menu -->
        <div id="dropdownfrm-{{ $discuss->id }}" class="z-10 hidden w-32  bg-mainclr divide-gray-100 rounded-lg shadow dark:bg-gray-700">
            <ul class="text-sm text-gray-700" aria-labelledby="dropdownForum">
              <li>
                @if (Request::is('favorites*'))
                  @if (isset($favorite) && $discuss->id)
                    <!-- Setelah tersimpan -->
                  ...
                  @endif  
                @else
                  @if (isset($favorite) && $favorite->isNotEmpty() && $favorite->first()->discuss_id)
                    <!-- Setelah tersimpan -->
              ...
              <li>
                <button class="flex pl-3 py-2 rounded-b-lg left-0 w-full show-modal text-white hover:bg-thirdclr dark:hover:bg-gray-600" type="button" data-modal-toggle="authentication1-modal-{{ $discuss->id }}">
