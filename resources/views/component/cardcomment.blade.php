...
{{-- card komen 1 --}}
  <div class="w-fit">
    ...
    {{-- ini dropdown abawah --}}  
    <button id="menu-btn2" data-dropdown-toggle="dropdown2-{{ $comment->id }}" class="absolute right-0 top-0 text-white hover:bg-thirdclr font-medium rounded-md text-sm px-2.5 py-2.5 text-center inline-flex items-center " type="button">
    ...
    <!-- Dropdown menu bawah -->
      <div id="dropdown2-{{ $comment->id }}" class=" hidden z-10 rounded divide-y shadow" data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="bottom" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate3d(246.667px, 70px, 0px);">
        <ul class="text-sm text-white bg-mainclr rounded-md" aria-labelledby="dropdownDefault">
          @if (auth()->user() && $comment->user_id == auth()->user()->id)
            <li>
              <div class="block py-2 px-4 hover:bg-thirdclr hover:rounded-t-md hover:text-secondaryclr">
                <button type="button" data-modal-toggle="EditComment-modal{{ $comment->id }}">
            ...
            <li>
              <div class="block py-2 px-4 hover:bg-thirdclr hover:text-secondaryclr">
          ...
          @endif
          <li>
            <div class="block py-2 px-4 hover:bg-thirdclr hover:rounded-b-md hover:text-secondaryclr">
              <button type="button" data-modal-toggle="ReportComment{{ $comment->id }}">  
