    ...
    <div>
      {{-- pivot --}}      
        <div class="mr-4 h-24">
            ...
            <p class="text-center text-lg my-2 text-white">{{ count($discuss->likes->whereIn('discuss_id', $discuss->id)) }}</p>
            <button class="mx-auto">
    ...
    <h1 class="font-bold text-3xl text-white">{{ $discuss->title }}</h1>
      ...
      <a href="">
        <img 
          ...
          alt=""/></a>
          <span class="pt-2 ml-1 hover:text-secondaryclr text-white">{{ count($discuss->comments) }}</span>
      </div>
      ...
      @if ($comment)
        <div >      
          <a href="#">
            <img
            class="w-8 absolute"
            src="{{ $comment->user->avatar ? asset('storage/'.$comment->user->avatar) : '/imgs/default/avatar.png' }}"
            >
          </a>
          {{-- card komen --}}
          ...
          <button class="text-secondaryclr hover:text-white">
            ...
            <span class="text-white mt-1.5 pl-1 pr-1 text-xs">{{ count($comment->likes->whereIn('comment_id', $comment->id)) }}</span>
          <button>
